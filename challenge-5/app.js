...
window.addEventListener("DOMContentLoaded", () => {
  ...
  checkboxes.forEach((checkbox) =>
    checkbox.addEventListener("click", selectCheckboxes)
  );

  let lastChecked;
  function selectCheckboxes(evt) {
    console.log("worked", this);
    let selectionRangeFlag = false;
    if (evt.shiftKey && this.checked) {
      checkboxes.forEach((checkbox) => {
        console.log("this in forEach", this);
        // Flip the flag variable to true when we check first item
        // then to false when we check the last item
        if (checkbox === this || checkbox === lastChecked) {
          selectionRangeFlag = !selectionRangeFlag;
        }
        if (selectionRangeFlag) checkbox.checked = true;
      });
    }
    lastChecked = this;
  }
